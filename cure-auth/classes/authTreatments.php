...
class authTreatments{
    ...
    public function find($user = null){
        if($user){
            $data = $this->_db->get('authorized_products',array('owner_id','=',$user));
            if($data->count()){
                return  $data->first();
            }
        }
    ...
    public function getOrgType($orgId){
        if($orgId){
            $data = $this->_db->get('authorized_products',array('product_id','=',$orgId));
            if($data->count()){
    ...
    public function orgStatus($user){
        if($user){
            $data = $this->_db->get('authorized_products',array('product_id','=',$user));
            if($data->count()){
